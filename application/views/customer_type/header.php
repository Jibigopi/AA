...
<head>
	...
	$(document).ready(function() {
          ...
          parsley_ext.onsubmit=function()
            { 
             ...
             $('#dt_table_tools').dataTable({
                                      "bProcessing": true,
				      "bServerSide": true,
                                      "sAjaxSource": "<?php echo base_url() ?>index.php/customer_type/data_table",
                                       aoColumns: [  
		...
		$("#add_new_stock").click(function() {
			if(document.getElementById('customer_type').value!="")
				  ...
				  $.ajax ({
					url: "<?php echo base_url('index.php/customer_type/add')?>",
					data: inputs,
                                        type:'POST',
			...
			$("#update_customer_type").click(function() {
			console.log(document.getElementById('ecustomer_type').value);
				if(document.getElementById('ecustomer_type').value!="")
               		{  inputs = {

					"ecustomer_type" : $('input[name=ecustomer_type]').val(),
					"customer_type_id" : $('input[name=customer_type_id]').val()
				  };
				 
				  $.ajax ({
					url: "<?php echo base_url('index.php/customer_type/update')?>",
					data: inputs,
                                        type:'POST',
					success: function(response) {
                                            if(response){
        ...
        function user_function(guid){
             ...
             if(result){
				 $.ajax({
					url: '<?php echo base_url() ?>index.php/customer_type/delete',
					type: "POST",
					data: {
						guid: guid
    ...
    function edit_customer_type(sid){
		   $.ajax({                                      
			  url: "<?php echo base_url() ?>index.php/customer_type/edit_customer_type/"+sid,                      
			  data: "",                       
											 
			  dataType: 'json',               
			  success: function(data)        
			  {            
				  document.getElementById('ecustomer_type').value=data[0]['type'];
				  document.getElementById('customer_type_id').value=data[0]['guid'];
			  } 
			});
    }   
</script>	
